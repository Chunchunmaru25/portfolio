...
import { Tag, Layers, Image as ImageIcon, Calendar, Plus, X, Trash2, EditIcon, ChevronLeft, ChevronRight } from "lucide-react";
...
// Number of screenshots rendered per page. Change this to test other
// page sizes (e.g. 4, 6) — everything else adapts automatically.
...
// ScreenshotCard
// ======================
// Extracted + memoized so that adding/removing/editing ONE screenshot
// (or unrelated state changes elsewhere in ProjectsView) doesn't force
// every other screenshot card in the page to re-render.
...
const ScreenshotCard = memo(
    ({ src, isDarkMode, onEdit, onDelete }: ScreenshotCardProps) => {
        return (
            <div
...
export default function ProjectsView() {
    ...
    // Tracks the current screenshot page per project. Defaults to page 1
    // when a project has no entry yet.
    ...
    const goToScreenshotPage = (projectId: string, totalSources: number, page: number) => {
        const totalPages = Math.max(1, Math.ceil(totalSources / SCREENSHOTS_PER_PAGE));
        const clamped = Math.min(Math.max(page, 1), totalPages);
        setScreenshotPages((prev) => ({ ...prev, [projectId]: clamped }));
    };
    ...
    const handleDeleteSource = async (projectId: string, sourceId: string) => {
        ...
        try {
            ...
            setScreenshotPages((prev) => {
                const current = prev[projectId];
                if (current === undefined) return prev;
                const remaining = (projects.find((p) => p.id === projectId)?.source.length ?? 1) - 1;
                const totalPages = Math.max(1, Math.ceil(remaining / SCREENSHOTS_PER_PAGE));
                return { ...prev, [projectId]: Math.min(current, totalPages) };
            });
                ...
                {!loading && !error && projects.length > 0 && (
                    <div className="space-y-6">
                        {projects.map((project) => {
                            const totalSources = project.source.length;
                            const totalPages = Math.max(1, Math.ceil(totalSources / SCREENSHOTS_PER_PAGE));
                            const currentPage = Math.min(
                                screenshotPages[project.id] ?? 1,
                                totalPages
                            );
                            const startIndex = (currentPage - 1) * SCREENSHOTS_PER_PAGE;
                            const visibleSources = project.source.slice(startIndex, startIndex + SCREENSHOTS_PER_PAGE);
                                                ...
                                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                                    {visibleSources.map((src) => (
                                                        <ScreenshotCard
                                                            ...
                                                            onDelete={(sourceId) => handleDeleteSource(project.id, sourceId)}
                                                        />
                                                    ))}
                                                </div>
                                                ...
                                                {totalPages > 1 && (
                                                    <div className="mt-3 flex items-center justify-center gap-3">
                                                        <button
                                                            ...
                                                            onClick={() => goToScreenshotPage(project.id, totalSources, currentPage - 1)}
                                                            className={`flex items-center gap-1 rounded-lg border border-dashed px-3 py-1.5 font-mono text-[11px] transition disabled:cursor-not-allowed disabled:opacity-30 ${isDarkMode
                                                                ? "border-[#3A4256] text-[#4B5468] hover:border-[#5B8DEF] hover:text-[#5B8DEF]"
                                                                : "border-gray-300 text-gray-400 hover:border-blue-400 hover:text-blue-500"
                                                                }`}
                                                        >
